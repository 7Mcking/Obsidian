---
kanban-plugin: board
people: Somak, Santinath, Sohom
tags:
  - mech
  - PL2
  - antenna
  - drone
  - satellite
---
## Santinath

- [ ] Design PL2 package with L and P Band #PL2 📅 2026-03-07 ⏫ #mech
- [ ] Design Hexacopter with L Band V2 📅 2026-03-02 🔺 🏁 keep 🆔 htwd5g #drone #urgent #important #PL1 #mech


## Somak

- [ ] Structural Analysis of Kaiju Drone 📅 2026-03-04 🔼 ➕ 2026-03-01 #drone #important #mech
- [ ] Get Vibration Inputs from Soma 📅 2026-03-02 ⏫ ➕ 2026-03-01 #drone #important #mech


## Sohom

- [ ] Get the CFRP plate manufactured for L Band payload flight on Hexacopter drone #urgent #important [due:: 2026-03-02] 
- [ ] 🔼 Stamping Machine Vendor List #important #generic
- [ ] Complete sheet metal manufacturing of electronic packages #urgent #PL2




%% kanban:settings
```
{"kanban-plugin":"board","list-collapse":[false,false],"show-checkboxes":true,"full-list-lane-width":true,"move-tags":true,"tag-colors":[],"archive-with-date":true,"date-picker-week-start":1,"hide-card-count":false,"lane-width":350,"table-sizing":{"card-tags":334,"lane":272,"card":648}}
```
%%